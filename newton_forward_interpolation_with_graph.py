...
for i in range(0,4):
    if(i==0):                                           
        add=add+(u[i][1]*y[b][i+1])               #udelta f(x)
    
    else:                                               
        add=add+((u[i][1]*y[b][i+1])/factorial(i+1))  #udelta2f(x)/factioal 2   upto n
...
st.use('ggplot')
plt.title('NEWTON FORWARD INTERPLOATION')          #matplotlib start
plt.xlabel('X')
plt.ylabel('Y')
plt.plot(x,t,label='x,f(x)')
plt.scatter(value,add,label='f(1.5)')
plt.legend()
plt.show()